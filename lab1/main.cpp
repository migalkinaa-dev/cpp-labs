...
int main()
{
    ...
    Rect *pR = new Rect(1,2,1,2);     // 2) Rect(int...)

    {
        Rect r2(r1);                  // 3) Rect(copy)

        Rect arRect[2];               // 4) Rect() для arRect[0]
                                      // 5) Rect() для arRect[1]

        for (int i = 0; i < 3; i++)
        {
            static Rect r3(i,i,i,i);  // 6) Rect(int...) ТОЛЬКО 1 РАЗ (при i=0)

            Rect r4(*pR);             // каждый раз: Rect(copy)  (всего 3 раза)
            Rect r5(i,i,i,i);         // каждый раз: Rect(int...) (всего 3 раза)

        } // в конце каждой итерации уничтожаются r5 и r4 (2 деструктора * 3 = 6)
    } // тут уничтожаются arRect[1], arRect[0], r2 (3 деструктора)

    delete pR;                        // деструктор для *pR (1 деструктор)
} // деструктор r1 (1 деструктор)

// а static r3 уничтожится в самом конце программы (после main)
 */
    /**
    ...
    // Функция-член (метод) принадлежит классу и вызывается для конкретного объекта.
    // Внутри метода есть скрытый указатель this на текущий объект, поэтому метод
    // имеет доступ к private-полям. Обычная функция не имеет this и не может
    // напрямую обращаться к private-полям объекта.
